...
class PandasDataset:
    ...
    def _pair_to_dataentry(self, item_id, df) -> DataEntry:
        ...
        if self.num_past_feat_dynamic_real > 0:
          ...
          entry["past_feat_dynamic_real"] = past_feat_dynamic_real.T[0][0]

        if self.num_past_feat_dynamic_real > 0:
            # past_feat_dynamic_real = df[self.past_feat_dynamic_real].values
            # if past_feat_dynamic_real.ndim == 2 and past_feat_dynamic_real.shape[1] == 1:
            #     past_feat_dynamic_real = past_feat_dynamic_real # Remove the extra dimension if past_feat_dynamic_real is a 2D array with one column
            # past_feat_dynamic_real = past_feat_dynamic_real[
            #     : len(past_feat_dynamic_real) - self.future_length
            # ]
