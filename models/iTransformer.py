...
class iTBackbone(nn.Module):
    def __init__(self, configs):
        super(iTBackbone, self).__init__()
    ...
    def forward(self, x): # B, L, D and B, L, 4 -> B, H, D
        B, _, D = x.shape
...
class iTransformer(nn.Module):
    def __init__(self, enc_in, seq_len, pred_len, d_model=512, n_heads=8, e_layers=3, d_ff=2048, factor=5,
                 dropout=0.1, activation="relu", output_attention=False):
        super(iTransformer, self).__init__()
        
        self.rev = iTRevIN(enc_in)
        ...
        self.backbone = iTBackbone(configs)
    ...
    def forward(self, x):
        ...
        y = self.backbone(x) # B, L, D and B, L, 4 -> B, H, D
